...
describe("parseFileUrlHref", () => {
  ...
  it("strips the drive slash only once an encoded colon is decoded", () => {
    const href = "file:///c%3A/Users/x/shot.png";
    expect(parseFileUrlHref(href)?.path).toBe("/c%3A/Users/x/shot.png");
    expect(parseFileUrlHref(href, { decodePath: true })?.path).toBe("c:/Users/x/shot.png");
  });

  it("keeps percent-encoding unless asked to decode", () => {
    const href = "file:///Users/julius/project/file%2520name.md";
    expect(parseFileUrlHref(href)?.path).toBe("/Users/julius/project/file%2520name.md");
    expect(parseFileUrlHref(href, { decodePath: true })?.path).toBe(
...
describe("parseMarkdownFileLink", () => {
  ...
  it("lets a client vouch for bare names it knows", () => {
    const options = { isKnownFileName: (name: string) => name === "Dockerfile" };
    expect(parseMarkdownFileLink("Dockerfile", options)?.path).toBe("Dockerfile");
    expect(parseMarkdownFileLink("Dockerfile:8", options)).toMatchObject({
      path: "Dockerfile",
      line: 8,
    });
    expect(parseMarkdownFileLink("Procfile", options)).toBeNull();
  });

  it("accepts conventional extensionless positions only when a client opts in", () => {
    const options = { acceptConventionalFilePosition: true };
    expect(parseMarkdownFileLink("Makefile:12", options)).toEqual({ path: "Makefile", line: 12 });
    expect(parseMarkdownFileLink("Makefile", options)).toBeNull();
    expect(parseMarkdownFileLink("TODO:12", options)).toBeNull();
  });
