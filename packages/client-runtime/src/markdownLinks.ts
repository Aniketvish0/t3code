...
/** Trims a link destination and unwraps the `<…>` form markdown uses for spaced paths. */
...
/**
 * Turns a `file:` URL into a host path. A non-localhost authority becomes a UNC
 * share. The path stays percent-encoded unless `decodePath` is set, so callers
 * that decode every destination in one place do not decode file URLs twice.
 */
  ...
  try {
    ...
    return {
      path: stripSlashPrefixedWindowsDrive(
        options?.decodePath ? safeDecodeURIComponent(rawPath) : rawPath,
      ),
      hash: parsed.hash,
    };
  } catch {
...
/**
 * Splits `path:12:5` or `path#L12C5` into its parts. A `:line` suffix wins over
 * the hash form, and non-positive numbers are dropped.
 */
...
export interface MarkdownFileLinkOptions {
  /** Lets a client vouch for bare extensionless names it has an icon for, like `Dockerfile`. */
  readonly isKnownFileName?: (name: string) => boolean;
  /**
   * Also accept conventional extensionless names with a `:line` suffix
   * (`Makefile:12`) as link destinations, not just as inline code.
   */
  readonly acceptConventionalFilePosition?: boolean;
}
...
function looksLikeFilePath(
  path: string,
  authoredPath: string,
  options: MarkdownFileLinkOptions | undefined,
): boolean {
  ...
  if (options?.isKnownFileName?.(path)) return true;
  if (options?.acceptConventionalFilePosition && isConventionalFilePosition(authoredPath)) {
    return true;
  }
  return RELATIVE_FILE_PATH_PATTERN.test(authoredPath) || RELATIVE_FILE_NAME_PATTERN.test(path);
}

/** `script.ts:10` is a file position, `https://x` and `mailto:x` are schemes. */
...
/**
 * Parses a markdown link destination into a file path and position, or null
 * when it is empty, an anchor, protocol-relative, an external scheme, or does
 * not look like a file. Relative paths are returned as written; resolving them
 * against a base directory is the caller's job.
 */
export function parseMarkdownFileLink(
  href: string,
  options?: MarkdownFileLinkOptions,
): FilePathPosition | null {
  ...
  const fileUrl = normalized.toLowerCase().startsWith("file:")
    ? parseFileUrlHref(normalized, { decodePath: true })
    : null;
  const source = fileUrl ?? splitMarkdownLinkSearchAndHash(normalized);
  const path = stripSlashPrefixedWindowsDrive(
    fileUrl ? source.path.trim() : safeDecodeURIComponent(source.path.trim()),
  );
  ...
  return looksLikeFilePath(position.path, path, options) ? position : null;
}
...
/**
 * The path relative to a workspace root, or null when it lies outside. Drive
 * letters and separators are normalized; the comparison ignores case because
 * both Windows and macOS default to case-insensitive filesystems.
 */
