...
const FILE_LINK_OPTIONS = {
  isKnownFileName: (name: string) => FILE_ICON_BY_NAME[name.toLowerCase()] !== undefined,
  acceptConventionalFilePosition: true,
};
...
export function resolveMarkdownLinkPresentation(href: string): MarkdownLinkPresentation {
  ...
  const target = parseMarkdownFileLink(normalized, FILE_LINK_OPTIONS);
  if (target) {
    return {
      kind: "file",
      href: normalized,
