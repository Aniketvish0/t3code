...
export const make = Effect.gen(function* () {
  ...
  return ManagedEndpointProvider.of({
    ...
    provision: Effect.fn("relay.managed_endpoint_provider.provision")(function* (input) {
      ...
      const tunnelAllocation = yield* allocations
        .recordTunnel({
          userId: input.userId,
          environmentId: input.environmentId,
          tunnelId: tunnel.id,
        ...
        Effect.mapError(
          (cause) =>
            new ManagedEndpointProvisioningFailed({
              ...
              tunnelName,
              tunnelId: tunnel.id,
              ...(tunnelAllocation.dnsRecordId === null
                ? {}
                : { dnsRecordId: tunnelAllocation.dnsRecordId }),
              cause,
            }),
        ),
      );
      const dnsAllocation = yield* allocations
        .recordDns({
          userId: input.userId,
          environmentId: input.environmentId,
          dnsRecordId,
          updatedAt: tunnelAllocation.updatedAt,
        })
        .pipe(
          Effect.mapError(
            (cause) =>
      ...
      const deprovisionTarget = yield* allocations
        .markReady({
          userId: input.userId,
          environmentId: input.environmentId,
          updatedAt: dnsAllocation.updatedAt,
        })
        .pipe(
          Effect.mapError(
            (cause) =>
